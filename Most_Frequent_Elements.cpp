...
{ 
    ...
    for (int i = 1; i < n; i++) { 
        if (arr[i] == arr[i - 1])                               //Since the array is sorted so to check frequncy elements will be complie on each other
            curr_count++; 
        else { 
    ...
    if (curr_count > max_count)                                  // If last element is most frequent 
    { 
        max_count = curr_count; 
        res = arr[n - 1]; 
    } 
...
int main()
{
	...
	cout<<"Most Frequent element in Array is: "<<mostFrequent(arr,n);
